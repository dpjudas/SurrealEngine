...
**Phase 7 step 0 — `Pickup`/`Inventory`/`Weapon` script contract (verified 2026-07-19, both Unreal Gold and UT99 GOTY, via `export scripts Engine`).** The two games are identical on every point below; nothing diverges this time (contrast phase 6's `SwitchWeapon` tail).

1. **`ValidTouch` does not trace, at all.** `Inventory.Pickup.ValidTouch` (`Inventory.uc`, state `Pickup`) is: `Other.bIsPawn && Pawn(Other).bIsPlayer && Pawn(Other).Health > 0 && Level.Game.PickupQuery(Pawn(Other), self)`, plus firing `Event` triggers if the pickup has one. No line trace, no distance check, nothing that would refuse a touch reaching through a wall. ⇒ **The wall guard in plan step 3.4 is required**, not optional.
2. **Gates besides player-ness:** `Health > 0` and `Level.Game.PickupQuery(...)` (the game-mode's own accept/reject hook — e.g. weaponstay rules, no-pickup gametypes). Both are opaque from C++ and must stay that way; the C++ filter's job is only to avoid dispatching where it's obviously wrong (a wall in the way), not to guess these.
3. **`Weapon.uc` has no `Pickup`-state override in either game** — weapons pick up through the exact same `Inventory.Pickup.Touch`/`ValidTouch` as every other item. One code path covers weapons and non-weapons alike.
4. **`Sleeping` state `ignores Touch`** (`State Sleeping { ignores Touch; ... }`) — a `CallEvent(actor, Touch, ...)` dispatched at a hidden/sleeping pickup is a guaranteed no-op at the language level, not just behaviourally unlikely. `bSleepTouch` is set only by `Sleeping.EndState`, which scans the actor's own `Touching[]` array — populated by the engine's own collision/move machinery, not by `VRHands`' direct `CallEvent` (see the `VRHands.h` header comment on why contacts are tracked out-of-band). **Consequence for D3:** a hand touching a sleeping pickup is inert either way — it neither collects the item nor sets `bSleepTouch` — so *not* filtering hidden pickups out of `Classify` costs nothing and is never wrong, it just doesn't add the body's `bSleepTouch` side effect either. Confirms D3's conclusion (don't filter on `bHidden`) but for a narrower reason: it's harmless-because-inert, not harmless-because-equivalent-to-the-body.
...
**When the Index *is* connected and SteamVR is already running (`pgrep -f vrcompositor` + `lsusb | grep -i "valve\|index"`), a real VR session can be driven unattended and it is worth more than the desktop-fallback check above.** Confirmed 2026-07-19 (phase 7): with `VR.Enabled=true` in `Settings.json`, `SE_AUTOLAUNCH=0 timeout 45 gdb -q -batch -ex run -ex "bt 20" -ex quit --args ./SurrealEngine "<gameRoot>"` drives the full XR pipeline (visible in stdout/stderr: `vrclient type=VRApplication_OpenXRInstance` → `Initializing CVulkanVRRenderer` → per-eye swapchain creation → `Transitioning XrSession from XR_SESSION_STATE_READY to XR_SESSION_STATE_SYNCHRONIZED` → repeated `xrEndFrame : projection view N has bounds ...`) and runs the whole per-frame VR path, including `VRHands::Tick`'s collision-overlap queries against the live level's actor set, for the full timeout with no crash. `exit 124` (timeout's own SIGTERM code) is the *success* signal here, not a failure - it means the process was still running cleanly when the clock ran out. This is strong automated evidence against crashes in code that runs every VR frame (widening the hand-collider whitelist, in phase 7's case), even though it cannot exercise anything that needs a human's hand actually moving into a pickup - grab feel, one-shot-not-per-frame behaviour, and wall-guard correctness still need the user in the headset.
