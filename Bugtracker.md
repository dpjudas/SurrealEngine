...
- "controller models" (the circles with a line) are to large, they should be 20% the size. They also dont need to be a spere, a circle with a line is enough
...
- on level transition (first to second level) the translater is lost - it is not in the item list anymore
    -> also see the bugs described in the docs
- translater model in the weapon wheel (an in the hand when selected) is fully black for some reason. The model of the flare looks correct too. 
