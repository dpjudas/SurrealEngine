...
std::vector<SweepHit> UModel::Sweep(const dvec3& origin, double tmin, const dvec3& dirNormalized, double tmax, const dvec3& extents, bool visibilityOnly)
{
	// Sort by closest hit
	std::vector<SweepHit> hits;
	Sweep(origin, tmin, dirNormalized, tmax, extents, visibilityOnly, &Nodes.front(), hits);
	std::stable_sort(hits.begin(), hits.end(), [](const auto& a, const auto& b) { return a.Fraction < b.Fraction; });
	return hits;
}
...
void UModel::Sweep(const dvec3& origin, double tmin, const dvec3& dirNormalized, double tmax, const dvec3& extents, bool visibilityOnly, BspNode* node, std::vector<SweepHit>& hits)
{
	if (node->CollisionBound >= 0)
	{
		int32_t* hullIndexList = &LeafHulls[node->CollisionBound];
		...
		for (int i = 0; i < hullPlanesCount; i++)
		{
			...
			BspNode* hullnode = &Nodes[hullIndex];
	...
	{
		Sweep(origin, tmin, dirNormalized, tmax, extents, visibilityOnly, &Nodes[node->Front], hits);
	}

	if (node->Back >= 0 && (startSide >= 0 || endSide >= 0))
	{
		Sweep(origin, tmin, dirNormalized, tmax, extents, visibilityOnly, &Nodes[node->Back], hits);
	}
}
...
double UModel::HullNodeAABBIntersect(const dvec3& origin, double tmin, const dvec3& dirNormalized, double tmax, const dvec3& extents, BspNode* node, bool flipPlane)
{
...
int UModel::NodeAABBOverlap(const dvec3& center, const dvec3& extents, BspNode* node)
{
	double e = extents.x * std::abs(node->PlaneX) + extents.y * std::abs(node->PlaneY) + extents.z * std::abs(node->PlaneZ);
