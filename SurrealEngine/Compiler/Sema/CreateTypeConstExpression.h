...
class CreateTypeConstExpression : AstExpressionVisitor
{
public:
	CreateTypeConstExpression(SemanticAnalysis &sema);
	TypeConstantExpression* exec(AstExpression *expression);
...
private:
	...
	T* createUnary(NodeType* node);

	template<typename T, typename NodeType>
	T* createBinary(NodeType* node);

	SemanticAnalysis &sema;
	TypeConstantExpression* constexp = nullptr;
};
