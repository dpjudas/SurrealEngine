...
#include "UI/Widgets/Scrollbar/Scrollbar.h"
...
ivec2 TextEdit::FindNextBreakCharacter(ivec2 search_start)
{
	search_start.x++;
...
ivec2 TextEdit::FindPreviousBreakCharacter(ivec2 search_start)
{
	search_start.x--;
	if (search_start.x <= 0)
...
ivec2 TextEdit::FromOffset(std::string::size_type offset) const
{
	int line_offset = 0;
...
ivec2 TextEdit::GetCharacterIndex(Point mouse_wincoords)
{
	Canvas* canvas = GetCanvas();
	for (size_t i = 0; i < lines.size(); i++)
	{
